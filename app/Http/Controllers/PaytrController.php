        ...
        if ((!empty(Auth::guard('customers')->user()) && $store->is_checkout_login_required == 'on') || $store->is_checkout_login_required == 'off') {
            try {
                ...
                if ($products) {
                    ...
                    $payment_amount = (float)($totalprice);
                    $merchant_oid = $orderID;
                    $user_name = $cust_details['name'];
                    $user_address = !empty($store->address) ? $store->address : 'no address';
                    $user_phone = !empty($store->whatsapp_number) ? $store->whatsapp : '0000000000';
                    ...
                    $post_vals = array(
                        ...
                        'user_address' => $user_address,
                        'user_phone' => $user_phone,
                        'merchant_ok_url' => route('store.pay.paytr.success', ['status' => $payment_success,  'slug' => $slug, 'amount' => $totalprice, 'product_id' => $product_id]),
                        'merchant_fail_url' => route('store.pay.paytr.success',['status' => $payment_failed,  'slug' => $slug, 'amount' => $totalprice, 'product_id' => $product_id]),
                        'timeout_limit' => $timeout_limit,
                        'currency' => $currency,
                        'test_mode' => $test_mode
                    );
