...
class XenditPaymentController extends Controller
{
        ...
        if ((!empty(Auth::guard('customers')->user()) && $store->is_checkout_login_required == 'on') || $store->is_checkout_login_required == 'off'){
            try {
                ...
                if ($products) {
                    ...
                    $xendit_token = $companyPaymentSetting['xendit_token'];
                    $xendit_api = $companyPaymentSetting['xendit_api'];
                    $currency = isset($store['currency_code']) ? $store['currency_code'] : 'USD';
                    $orderID = strtoupper(str_replace('.', '', uniqid('', true)));

                    $response = ['slug'=>$slug, 'orderId' => $orderID, 'get_amount' => $totalprice, 'product_id' => $product_id];
                    Xendit::setApiKey($xendit_api);
                    $params = [
                        'external_id' => $orderID,
                        'payer_email' => $cust_details['email'],
                        'description' => 'Payment for order ' . $orderID,
                        'amount' => $totalprice,
    ...
    public function getStorePaymentStatus(Request $request)
    {
        ...
        $slug       = $fixedData['slug'];
        $getAmount  = $fixedData['get_amount'];
        $product_id = $fixedData['product_id'];
